 ...
 * @author Admin
 */
@WebServlet(name = "ReleaseOrder", urlPatterns = {"/ReleaseOrder"})
public class ReleaseOrder extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String [] checked = request.getParameterValues("checkorder");
        
        ArrayList<Integer> allOrder = new ArrayList<Integer>();
        for(int i=0;i<checked.length;i++){
            allOrder.add(Integer.parseInt(checked[i]));
        }
        
        DAO dao = new DAO();
        dao.deleteOrders(allOrder);
